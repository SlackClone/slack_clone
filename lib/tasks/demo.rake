namespace :demo do
  ...
  task initial: :environment do
    ...
    # CH general(all), GitHub_Msg(all), HEAD(龍哥、主管-昶宏)
    ...
    ws.channels.find_by(name: "HEAD").users = User.where(nickname: ["龍哥", "主管-昶宏"])
    ...
    # 最後需要什麼
    # workspace：百倍紅寶石
    # Channel：general(all), HEAD(龍哥、主管-昶宏), GitHub_Msg(all)
    # User：「龍哥」、「主管-昶宏」、「Ruby」(還沒加入WS)、「Keddie」、「Sabrina」「CatBot」
    # Message：專案事項(HEAD)
